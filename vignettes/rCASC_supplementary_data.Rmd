...
In Figure \ref{fig:fig.31} are summarized the similarities and differences between rCASC and the other three workflows. rCASC is the only one providing support at fastq level, as all the others require a processed counts table. rCASC and SCell data preprocessing offer more options than the others. The fours workflow implement very different data reduction and clustering methods. However, only rCASC performs clustering in presence of data perturbation and it is also the only one that provides measurement for cluster quality using silhuette score, which measures the consistency within clusters of data,  and Cells Stability Score, which measures the persistence of each cell in a cluster upon data perturbation. 
...
Considering reproducibility only rCASC provides both computational and functional reproducibility. Both Granatum and Scell perform data manipulation based on graphical tools, e.g. cell outlyers removal in Granatum and genes filtering in SCell preprocessing, which are not trackable, thus not providing functional reproducibility. On the other side simpleSingleCell package cannot guarantee computational reproducibility unless it is embedded in a virtual machine or an other type of virtual container , e.g. docker.

In conclusion, from the above comparisons, rCASC results to be a workflow with valuable new features that could help researchers in defining cells sub-populations and detecting sub-population specific markers, under the umbrealla of data reproducibility.
