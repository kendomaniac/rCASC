...
#### Some considerations about the number of reads/UMI to be used in a single-cell sequencing experiment.
...
## Estimating the number of cluster to be used for cell sub-population discovery.

The CASC core clustering tool is [*SIMLR*](https://www.ncbi.nlm.nih.gov/pubmed/28263960), which requires that the user provides the number of clusters in the cells under analysis will aggregate. Unfortunately, there is no definitive answer to this question and it represents the [*weakest aspects of performing cluster analysis*](https://hlab.stanford.edu/brian/number_of_clusters_.html). [*There are various way to try to address this issue*](http://www.sthda.com/english/articles/29-cluster-validation-essentials/96-determining-the-optimal-number-of-clusters-3-must-know-methods/), we tested [*silhouette*](https://www.sciencedirect.com/science/article/pii/0377042787901257?via%3Dihub) and [gap statistics](http://web.stanford.edu/~hastie/Papers/gap.pdf), but none of the two approches resulted to be particularly effective also because we are non simply interested in the fine the optimal numver o clusters, but we also want to know if any perturbation in the dataset, e.g. removal of a random subset of cells, is affecting the number of clusters.
Now a day the best approach we could find is the use of [*griph*](https://ppapasaikas.github.io/griph/), which is a hierarchical clustering algorithm. In brief, griph build a dendrogram and defines the place in which dendrogram is cut, i.e. defining the number of clusters, running a top-down similarity analysis. As indicated above we are interested to identify not only the optimal cluster number but also if the cluster number is affected by removal of a random subset of cells.

The function **clusterNgriph** evaluates the number of cluster in which a set of cells will aggregate upon leave-N-out bootstrap. To provide an example of how **clusterNgriph** we will use a dataset made of 100 cells from [*Zheng* 2016](https://community.10xgenomics.com/t5/Data-Sharing/10x-Single-Cell-3-Paper-Zheng-et-al-2016-Datasets/td-p/231) paper:
