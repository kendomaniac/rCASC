...
counts <- read.table("annotated_setPace_10000_noC5.txt", sep="\t", header=T, row.names=1, stringsAsFactors = F)
#changing separator underscore
names(counts) <- gsub("_","-",names(counts))
clusters <- read.table("annotated_setPace_10000_noC5_clustering.output.txt", sep="\t", header=T, row.names=1, stringsAsFactors = F)
#changing separator underscore
rownames(clusters) <- gsub("_","-",rownames(clusters))
#checking that counts and clusters cell IDs are in the same order
identical(names(counts), rownames(clusters))
#adding the cluster number as covariate to the cells
names(counts) <- paste(names(counts), clusters$Belonging_Cluster, sep="_")
ref <- counts[,grep("_3$", names(counts))]#92
others <- counts[,setdiff(seq(1,dim(counts)[2]),grep("_3$", names(counts)))]#445
#ordering the cluster covariates to have the reference cluster as first set of columns in the counts table
counts <- data.frame(ref, others, check.names = F)#537
write.table(counts, "annotated_setPace_10000_noC5_reordered.txt", sep="\t", col.names = NA)

#detecting differentially expressed genes with edgeR ANOVA-like
deDetection(group="docker", data.folder=getwd(), counts.table="annotated_setPace_10000_noC5_reordered.txt", file.type="txt", logFC.threshold=1, FDR.threshold=0.05, logCPM.threshold=4, plot=TRUE)#2472

# selecting only the UP genes in C1,2,4,5
up <- read.table("UP_only_filtered_DE_annotated_setPace_10000_noC5_reordered.txt", sep="\t", header=F, stringsAsFactors = F)

de <- read.table("filtered_DE_annotated_setPace_10000_noC5_reordered.txt", sep="\t", header=T, row.names = 1)
de.up <- de[which(rownames(de)%in%up$V2),]#445
...
## **Section 7** Performace in SeqBox

We run various performance tests on [*SeqBox*](https://www.ncbi.nlm.nih.gov/pubmed/29069297) ecosystem (Intel NUC6I7KYK miniPC equipped with 32 GB RAM and 500 GB SSD).
