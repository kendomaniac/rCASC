...
As described in section 4, the peculiarity of CASC is the presence of user definable bootstrap procedure. CASC handles the results of bootstraps via a cell stability score (Figure \ref{fig:fig.19}). In brief, a set of cells to be organized in cluster  (Figure \ref{fig:fig.19}A) is analysed with SIMLR is applied using a users defined k number of cluster (Figure \ref{fig:fig.19}B). A user defined \% of cells is removed form the original data set and these cells are cluster again (Figure \ref{fig:fig.19}C). The clusters obtained in the bootstrap step are compared with the clusters generated on the full dataset using Jaccard index (Figure \ref{fig:fig.19}D-E) and only if the Jaccard index is at least 0.8 the cluster is called confirmed in the bootstrap step (Figure \ref{fig:fig.19}F) and to the cells belonging to that cluster is added the value 1 (Figure \ref{fig:fig.19}G). On the basis of the results obtained with few hundreds bootstraps cells are labelled with different symbols to indicate their stability in a specific cluster (Figure \ref{fig:fig.19}H).
