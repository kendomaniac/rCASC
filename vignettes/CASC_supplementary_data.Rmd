...
- *Parameters* (only those without default, for the full list of parameters, please refer to the function help):
    ...
    + *scratch.folder*,	the folder located on a fast hardrive, i.e. SSD, in which all the I/O will be located. Having the scratch folder on a SSD strongly reduce the analysis time.	
...
The function, **cellrangerCount** allows to go from fastq files to counts table. Tis function implments [*Cellranger*](https://support.10xgenomics.com/single-cell-gene-expression/software/pipelines/latest/algorithms/overview) , the 10xGenomics tool allowing the conversion of the fastq, generated with 10XGenomics platform, into a count matrix. 
...
The output is a PDF providing zeros distributions before and after removal of all genes without counts. A tab delimited file with the prefix **filtered\_** in which the filtered data are saved.
...
The output is a pdf named **genes.umi.pdf** where each dot represent a cell. X axis is the total number of reads mapped on each cell il log10 format and Y axis is the number of detected genes. 
...
Figure \ref{fig:fig.6.2} clearly show that the number of genes detectable by 10XGenomics sequencing (Figure \ref{fig:fig.6.2}A-C) is far less of those detectable using a whole transcript experiment (Figure \ref{fig:fig.6.2}D-F). It is notable that the the number of genes, detectable with 3' end sequencing, is located in the lower range of those obtainable with whole transcript sequencing, i.e in case of 25K reads/cells in 3' end sequencing, Figure \ref{fig:fig.6.2}A, the max number of genes detected is 350, which is in the range of the lowest number of genes detectable with 25K reads/cell in whole transcript sequencing, Figure \ref{fig:fig.6.2}D. The critical point is that it is difficult motivate why, in 3' end sequencing, the number of detectable genes/cell is so scattered. As far of our knowledge, today is not possible to understand if such scattere number of genes has a biological meaning or if it is an artifact of the technology. However, it has to be noted that cells with a low number of genes called present will have a gene repertoire made mainly of housekeeping genes, ribosomal and mitohondrial genes. Thus, these cells will be difficult to cluster on the basis of cell-type-specific transcription profile.  
...
To evaluate what would be the minimal number of genes/cell, which guarantee sufficient transcripts information for clusters we have used a subset of [*Zheng* 2016](https://community.10xgenomics.com/t5/Data-Sharing/10x-Single-Cell-3-Paper-Zheng-et-al-2016-Datasets/td-p/231) paper data. 

setA is made of the following cell type:
    + B-cells (25K reads/cell), 
    + Monocytes  (100K reads/cell), 
    + Stem cells (24.7K reads/cell), 
    + Natural Killer cells (29K reads/cell), 
    + Naive T-cells (19K reads/cell)
    
setA is made of 100 cells selected from each of the 
...
### **Section 7.2** EdgeR ANOVA-like analysis
...
```{r fig.26, fig.cap="EnrichR analysis. A) Transcription specific enriched paths cluster 1. B) KEGG specific enriched paths cluster 1.  A) Transcription specific enriched paths cluster 2. B) GO Biological Process specific enriched paths cluster 2.", echo=FALSE, eval=TRUE, out.width="50%", fig.align="center"}
library(knitr)
include_graphics('../inst/img/c1_specific1.jpeg')
```
