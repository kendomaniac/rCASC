...
```{r fig.12, fig.cap="reCAT prediction of cell cycle in Buettner et al. (Nat.Biotechnol. 2015) dataset. Figure extracted from reCAT github", echo=FALSE, eval=TRUE, out.width="50%", fig.align="center"}
library(knitr)
...
**reCAT** prediction step is implemented in CASC in the function **recatPrediction** which require a data set annotated using scannobyGtf.

**ccRemover** is implemented in CASC in the function **ccRemove**, which require a data set annotated using scannobyGtf. **IMPORTANT**: The output of ccRemover does not require log transformation to be used in clustering analysis.
...
recatPrediction(group="docker",scratch.folder,file=paste(getwd(), "annotated_buettner_G1G2MS_counts_10000.txt", sep="/"), separator="\t", geneNameControl=1, window=10, seed=111)
...
The analysis above took XXX mins on an [SeqBox](www.seqbox.com), equipped with an Intel i7-6770HQ (8 threads), 32 Gb RAM and 1Tb SSD. 
