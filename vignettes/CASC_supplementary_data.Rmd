...
- *Parameters* (only those without default, for the full list of parameters, please refer to the function help):
    ...
    + *index.folder*,	the folder where inDrop reference genome was created with index was created with indropIndex.
...
```{r fig.5, fig.cap="Zeros distribution in full table, orange, and filtered table, blue", echo=FALSE, eval=TRUE, out.width="40%", fig.align="center"}
library(knitr)
...
To estimate the overall amount of genes detectable in each cell, the function *genesUmi* generates a plot of the number of genes present in a cell with respect to the total number of UMI. The number of UMIs required to indicate that a gene is called present in a cell is a parameter defined by user. We suggest to use at least 3 UMI as minimal threshold to count a gene present in a cell.
...
Sequencing depth, which affect differential expression analysis or sub-population partitioning, affects the structure of a single-sequencing dataset at two levels: 

- number of genes called present in the experiment

- robustness of gene expression, i.e. number of reads associated to a gene. 

In particular, the number of genes called present in the experiment is the key element for the discrimination between sub-populations. In Figure \ref{fig:fig.6.2} it shown the effect of sequencing depth on the number of detectable genes in a set of 10XGenomics sequencing experiments (25K sequenced reads/cell extracted from CD19 B-cells [[*Zheng et al*](https://community.10xgenomics.com/t5/Data-Sharing/10x-Single-Cell-3-Paper-Zheng-et-al-2016-Datasets/td-p/231)],  83K sequenced reads/cell extracted from GSM2833284_Naive_WT_Rep1 [[*Pace et al. 2018*](https://www.ncbi.nlm.nih.gov/pubmed/29326266)], 250K sequenced reads/cell from an unpublished mouse experiment), 25K, 100K and 250K sequenced reads/cell from a unpublished whole transcript human single-cell sequencing done on Fluidigm C1. 
3 UMI are used as minimal threshold to call present a gene in 10XGenomics experiments and 5 reads [[*Tarazona et al. 2011*](https://genome.cshlp.org/content/21/12/2213.long)] as minimal threshold to call a gene present in a single cell whole trasncriptome experiment.  
...
Figure \ref{fig:fig.6.2} clearly shows that the number of genes detectable by 10XGenomics sequencing (Figure \ref{fig:fig.6.2}A-C) is far less of those detectable using a whole transcript experiment (Figure \ref{fig:fig.6.2}D-F). In the case of 25K reads/cells in 3' end sequencing (Figure \ref{fig:fig.6.2}A) the number of called genes goes from a doxen of genes to 350. In the corresponding whole transcript sequencing experiment (Figure \ref{fig:fig.6.2}D), 350 genes is the lowest number of genes detectable in a cell. At 250K read/cell the range of detectable genes in 3' end experiment (few hunders genes to 2000 genes)  and in whole transcripts experiments (less than 1000 genes to 3500 genes) are more similar. 
The larger scatterring and the overall lower number of detectable genes in 3' end sequencing experiment, with respect to whole transcript experiments, is a peculiarity of the technology [[*Ziegenhain and coworkers*](https://www.ncbi.nlm.nih.gov/pubmed/28212749)].  
It has also to be highlighted that cells with a very low number of genes called present will have a gene repertoire made mainly of housekeeping genes, ribosomal and mitochondrial genes, see Figure \ref{fig:fig.8} in **Section 3.4**. Thus, the lack of cell-type specific genes make latter cells useless for the identification of functional cell sub-populations.  
...
We try to address the following two points:

- How the number of total UMIs/cell is affecting the separation between sub-populations?

- A threshold for the minimal mumber of UMIs/cell required to discriminate between cell sub-populations can be suggested?

To address the above points we used two types of cells belonging to the T-cells to make the experiment similar to what could be expected in a real experiment for sub-populations clustering. The T-cytotoxic (10209 cells, Figure \ref{fig:fig.27}A) and T-regulatory (10263, Figure \ref{fig:fig.27}B) cells extracted from [*Zheng* 2016](https://community.10xgenomics.com/t5/Data-Sharing/10x-Single-Cell-3-Paper-Zheng-et-al-2016-Datasets/td-p/231) paper were used. The two sets of cells were sequenced by Zheng with a coverage of approximately 21K reads/cell. 

We generated three datasets:

- **d3.4**, which is made of 100 cells randomly selected within cells having, in each of the two datasets, a total UMIs/cell value greater than 2511.

- **d3**, which is made of 100 cells randomly selected within cells having, in each of the two datasets, a total UMIs/cell value comprised between 2511 and 1000.

- **d3m**, which is made of 100 cells randomly selected within cells having, in each of the two datasets, a total UMIs/cell smaller than 1000.
...
The above observations indicates that 3' end sequencing has a much lower genes called present (Figure \ref{fig:fig.6.2}) with respect to whole transcript sequencing. However, since the number of sequenced cells is more higher in 3' end sequencing, users could decide to selected for the clustering the subset of cells with the highest number of genes called present. 
...
- *Parameters* (only those without default, for the full list of parameters, please refer to the function help):
    ...
    + *p_value*,	 lorenz statistics threshold, suggested value 0.05, i.e. 5% probability that the cell of low quality is selected
...
In figure \ref{fig:fig.8} is shown the effect of the removal of both mitocondrial and ribosomal protein genes. It is notable that, in this specific experiment, in cells with less than 1000 UMI nearly all detected genes were only mitocondrial and ribosomal protein genes. This observation further support the observation in Figure \ref{fig:fig.27}C, showing that the genes detectable with 1000 UMIs/cell are not sufficient to discriminate between two different cell types.
This filter is suitable to identify cells which do not contain any informative gene other the mitocondrial and ribosomal proteins. However, in case the difference between resting and actively proliferating cells is an important element of cell sub-population discovery this filter should not be applied.
...
- *Parameters* (only those without default, for the full list of parameters, please refer to the function help):
    ...
    + *rawCount*,	1 for raw 0 otherwise
...
- *Parameters* (only those without default, for the full list of parameters, please refer to the function help):
    ...
    + *percent*,	percentage of cells randomly removed in each permutation
    + *separator*,	separator used in count file, e.g. '\\t', ','
...
To observe the effect of the reduced dissimilarity between sub-populations on the stability of the number of clusters, we use the CASC **clusterNgriph** function on the above mentioned 4 datasets using 160 permutations/each, and randomly removing in each permutation 10\% of the cells. Each analysis took approximately 60 mins on a SeqBox hardware. 
...
Using the function **clustersFeatures** it is possible to select the genes that are more up-regulated in a specific cluster. This function focus only on upregulated genes because the ANOVA-like procedure is comparing compares all clusters with respect to a reference cluster. Thus, up-regulated genes are genes specific of the cluster under analysis as those down-regulated are characteristics of the reference cluster. The delta parameter indicate the minimal distance existing, for a specific gene average logFC in a specific cluster, with respect to all the other clusters under analysis.
...
## **Section 7** An example of CASC analysis: PACE et al. Science 2018.
...
We detected a total of 2742 differentially expressed genes using the naive cluster 3 as reference, Figure \ref{fig:fig.25}C. 
...
Then, the following analysis focus only on up-regulated genes, i.e. those gene specifically mor expressed in a cluster with respect ot the naive cluster used as reference in the ANOVA-like.
