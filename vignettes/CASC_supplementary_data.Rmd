...
author: "Luca Alessandrì, Marco Beccuti, Maddalena Arigoni, Martina Olivero, Greta Romano, Francesca Cordero and Raffaele A Calogero"
date: "8/21/2018"
output: pdf_document
...
- **10XGenomics**:
    ...
    + From fastq to UMI counts using cellranger: **cellranger_count**
...
Assuming the above example paths, the cell counts matrices **results_cellranger.cvs** and **results_cellranger.txt** are located in /data/test_cell_ranger/results_cellranger/outs/filtered_gene_bc_matrices folder.
...
The function **filterZeros** retains all genes that have cells without a user defined fraction of zeros (between 0 and 1, where 1 indicate only genes without 0s are retained, and 0 insted indicates that genes with at least a value different from zero are retained) and plot the frequency distribution of genes with counts in the cells. 
...
To estimate the overall amount of genes detectable in each cell, the function *genesUmi* produces a plot of the genes number with respect to total number of UMI for each cell. The number of UMIs required to indicate that a gene is detected in a cell are defined by user. We suggest to use 3 UMI as minimal threshold to consider a gene called present in a cell.
...
```{r fig.10, fig.cap="Effect of the SCnorm on the dataset in Figure \ref{fig:fig.9}", echo=FALSE, eval=TRUE, out.width="40%", fig.align="center"}
library(knitr)
...
**ccRemover** is implmented in CASC in the function **ccRemove**, which require a data set annotated using scannobyGtf. As toy example we used 80 cells respectively from G1 G2M and S cells from the dataset published by [*Buettner et al Nat.Biotechnol. 2015*](https://www.ncbi.nlm.nih.gov/pubmed/25599176), which contains naive-T-cells and T-helper2-cells mixed togheter and sorted on the basis of the cell cycle condition.

**IMPORTANT**: The output of ccRemover does not require log transformation to be used in clustering analysis.
\newline
```{r, echo=TRUE, eval=FALSE}

 system("wget http://130.192.119.59/public/buettner_20cells_G1G2MS_counts.txt.zip")
 unzip("buettner_20cells_G1G2MS_counts.txt.zip")

scannobyGtf(group="docker", file=paste(getwd(),"buettner_20cells_G1G2MS_counts.txt",sep="/"),
                    gtf.name="Mus_musculus.GRCm38.92.gtf", biotype="protein_coding", 
                    mt=TRUE, ribo.proteins=TRUE,umiXgene=3)

topx(data.folder=getwd(),file.name="annotated_buettner_20cells_G1G2MS_counts.txt",threshold=10000, logged=FALSE)

ccRemove(group="docker" , scratch.folder="/data/scratch",
        file=paste(getwd(),"annotated_buettner_20cells_G1G2MS_counts_10000.txt", sep="/"), separator="\t",
        seed=111, cutoff=3, species="mouse", rawCount=1)

permutationClustering(group="docker", scratch.folder="/data/scratch", file=paste(getwd(), "annotated_buettner_20cells_G1G2MS_counts_10000.txt", sep="/"), nPerm=8, permAtTime=8, percent=10, range1=3, range2=3, separator="\t", logTen=0, clustering="tSne", perplexity=10, seed=1111, rK=0)


permutationClustering(group="docker", scratch.folder="/data/scratch", file=paste(getwd(), "LS_cc_annotated_buettner_20cells_G1G2MS_counts_10000.txt", sep="/"), nPerm=80, permAtTime=8, percent=10, range1=3, range2=3, separator="\t", logTen=1, clustering="tSne", perplexity=10, seed=1111, rK=0)

```

The analysis done above took 1.5 mins on an [SeqBox](www.seqbox.com), equipped with an Intel i7-6770HQ (8 threads), 32 Gb RAM and 1Tb SSD. 
