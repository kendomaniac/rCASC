...
#' @param file, a character string indicating the counts table file. IMPORTANT in the header of the file the covariate group MUST be associated to the column name using underscore, e.g. cell1_cov1
...
#'
#' @export
deTwoGroups <- function(group=c("sudo","docker"), file){
...
counts.table=paste(matrixName,".",file.type,sep="")  #running time 1
  ptm <- proc.time()
  ...
  params <- paste("--cidfile ",data.folder,"/dockerID -v ", data.folder, ":/data -d docker.io/repbioinfo/desc.2018.01 Rscript /bin/desc2.R ", counts.table, " ", file.type, sep="")
