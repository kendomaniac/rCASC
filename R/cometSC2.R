#' @title Marker genes discovery with COMETSC for autoencoder results
...
#' @param clustering.output, path of the clustering.output file to use
...
#'
#' @export
cometsc2 <- function(group=c("sudo","docker"), file, scratch.folder, threads=1,  X=0.15, K=2, counts=c("True", "False"), skipvis=c("True", "False"), nCluster, separator,clustering.output){
  ...
  system(paste("mkdir ",scrat_tmp.folder,"/",matrixName,sep=""))
  system(paste("mkdir ",scrat_tmp.folder,"/",matrixName,"/",nCluster,sep=""))
  system(paste("cp ",clustering.output," ",scrat_tmp.folder,"/",matrixName,"/",nCluster,"/",matrixName,"_clustering.output.",format,sep=""))
  ...
  if(resultRun==0){
    system(paste("cp -R ", scrat_tmp.folder,"/",matrixName,"/",nCluster,"/output* ",data.folder,"/Results/", sep=""))
  }
  #running time 2
  ptm <- proc.time() - ptm
  dir <- dir(data.folder)
