...
logTransform <- function(group=c("sudo","docker"),file,separator, overdispersion=0.05,pseudo_count=1/(4*overdispersion),size_factors=TRUE,minimum_overdispersion=0.001, scratch.folder,outputFolder){
  ...
  params <- paste("--cidfile ",data.folder1,"/dockerID -v ",scrat_tmp.folder,":/scratch -v ", data.folder1, ":/data -d repbioinfo/transformgampoi Rscript /home/main.R ",separator," ",format1," ",matrixName1," ",overdispersion," ",pseudo_count," ",minimum_overdispersion," ",size_factors,sep="")
