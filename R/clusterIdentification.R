...
system(paste("cp -r ",data.folder,"/Results/* ",scrat_tmp.folder,sep=""))
  ...
  if(group=="sudo"){
    params <- paste("--cidfile ",data.folder,"/dockerID -v ",scrat_tmp.folder,":/scratch -v ", data.folder, ":/data -d docker.io/rcaloger/clusteridentification Rscript /home/main.R ",matrixName," ",nCluster," ",format," ",separator,sep="")
    resultRun <- runDocker(group="sudo",container="docker.io/rcaloger/clusteridentification", params=params)
  }else{
    params <- paste("--cidfile ",data.folder,"/dockerID -v ",scrat_tmp.folder,":/scratch -v ", data.folder, ":/data -d docker.io/rcaloger/clusteridentification Rscript /home/main.R ",matrixName," ",nCluster," ",format," ",separator,sep="")
    resultRun <- runDocker(group="docker",container="docker.io/rcaloger/clusteridentification", params=params)
  }
