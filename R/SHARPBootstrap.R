...
sharpBootstrap <- function(group=c("sudo","docker"), scratch.folder, file, nPerm, permAtTime, percent, separator, logTen=0, seed=111){

 
   permutationClusteringSHARP(group=group,scratch.folder=scratch.folder,file=file,nPerm=nPerm,permAtTime=permAtTime,percent=percent,separator=separator,logTen=logTen,seed=seed)
   if(separator == ","){
   ...
   cluster <- as.numeric(list.dirs(cluster.path, full.names = FALSE, recursive = FALSE))
    ...
    if(length(cluster)==1){
       ...
       cat("Check the results generated by clusterNgriph, and run permAnalysisGriph selecting the cluster of interest\n")
