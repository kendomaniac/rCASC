#' @title Evaluation of the numbur of optimal clusters
...
#' @param logTen, 0 for raw count, 1 otherwise
#' @param nPerm, how many times number of cluster will be evaluated
#' @param percent, number of cells that has to be removed for bootstrap
#' @param rangeFile, name of the file that has for each row each number of cluster that has to be analyzed
...
#'}
#' @export
nClusterEvaluationSIMLR <- function(group=c("sudo","docker"), scratch.folder, file,range1,range2,separator,logTen,nPerm,percent,rangeFile){
