...
#'}
#' @export
seuratPCAEval <- function(group=c("sudo","docker"), scratch.folder, file, separator, logTen=0,seed=1111,sparse=FALSE,format="NULL"){
if(!sparse){
...
system(paste("cp ",data.folder,"/",matrixName,".",format," ",scrat_tmp.folder,"/",sep=""))
  ...
  #executing the docker job
    params <- paste("--cidfile ",data.folder,"/dockerID -v ",scrat_tmp.folder,":/scratch -v ", data.folder, ":/data -d docker.io/rcaloger/seuratpcaeval Rscript /home/main.R ",matrixName," ",format," ",separator," ",logTen," ", seed, sep="")
