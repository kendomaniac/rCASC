...
#'}
#' @export
harmony <- function(group=c("sudo","docker"), scratch.folder, file,separator,meta_data,usePCA){
...
data.folderT=dirname(meta_data)
positionsT=length(strsplit(basename(meta_data),"\\.")[[1]])
matrixNameCT=strsplit(basename(meta_data),"\\.")[[1]]
matrixNameT=paste(matrixNameCT[seq(1,positionsT-1)],collapse="")
formatT=strsplit(basename(basename(meta_data)),"\\.")[[1]][positionsT]

system(paste("cp ",data.folderT,"/",matrixNameT,".",formatT," ",scrat_tmp.folder,sep=""))




  #executing the docker job
    params <- paste("--cidfile ",data.folder,"/dockerID -v ",scrat_tmp.folder,":/scratch -v ", data.folder, ":/data -d repbioinfo/harmony Rscript /home/main.R ",matrixName," ",format," ",separator," ",matrixNameT," ",usePCA,sep="")
