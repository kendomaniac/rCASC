...
#' @param clusterMethod, clustering methods: "GRIPH","SIMLR","SEURAT","SHARP","KMEANS","HCLUST","FLOW"
...
#'}
#' @export
autocluster4clustering <- function(group=c("sudo","docker"), scratch.folder, file, separator, nCluster, projectName, clusterMethod=c("GRIPH","SIMLR","SEURAT","SHARP","KMEANS","HCLUST","FLOW"),seed=1111,pcaDimensions=20,perplexity=20){
...
system(paste("cp -r ",data.folder," ",scrat_tmp.folder,"/",sep=""))
  ...
  if(clusterMethod=="GRIPH"){
    ...
    if(clusterMethod=="FLOW"){
    params <- paste("--cidfile ",data.folder,"/dockerID -v ",scrat_tmp.folder,":/scratch:Z -v ", data.folder, ":/data -d repbioinfo/flowgrid Rscript /home/mainFLOW.R ",projectName," ",matrixName," ",separator," ",seed,sep="")
}
  if(clusterMethod=="HCLUST"){
    params <- paste("--cidfile ",data.folder,"/dockerID -v ",scrat_tmp.folder,":/scratch:Z -v ", data.folder, ":/data -d repbioinfo/simlrgriphauto Rscript /home/mainHclust.R ",projectName," ",matrixName," ",separator," ",nCluster," ",seed,sep="")
}
  ...
  if(clusterMethod=="SHARP"){
    params <- paste("--cidfile ",data.folder,"/dockerID -v ",scrat_tmp.folder,":/scratch:Z -v ", data.folder, ":/data -d repbioinfo/sharpforauto Rscript /home/mainSHARP.R ",projectName," ",matrixName," ",separator," ",seed,sep="")
}

 if(clusterMethod=="SEURAT"){
    params <- paste("--cidfile ",data.folder,"/dockerID -v ",scrat_tmp.folder,":/scratch:Z -v ", data.folder, ":/data -d docker.io/repbioinfo/seuratautoclusters Rscript /home/mainSeurat.R ",projectName," ",matrixName," ",separator," ",pcaDimensions," ",seed,sep="")
}
