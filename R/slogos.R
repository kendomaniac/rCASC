...
logosR <- function(group=c("sudo","docker"),scratch.folder,fastqPath,resFolderCustom="NULL"){
  ...
  params <- paste("--cidfile ",genomeFolder,"/dockerID  -v ", scrat_tmp.folder, ":/scratch -d ",dockerImage, " /home/logoscript.sh /scratch",sep="")
  ...
  if(resultRun==0){
    ...
    dir.create(res)
    system(paste("cp -R ", scrat_tmp.folder, "/* ",res, sep=""))
